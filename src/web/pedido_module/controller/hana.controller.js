...
const getAllArticulos= async (itemName) => {
    try {
        ...
        const query = `select * from ${process.env.PRD}.ifa_dm_articulos where (upper("ItemName") LIKE '%${itemName}%' or upper("ItemCode") LIKE '%${itemName}%') and "ItmsGrpCod" = 105 order by "ItemName" limit 15`;
        console.log({ query })
        const result = await executeQuery(query)
        return result
    } catch (error) {
