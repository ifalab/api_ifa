...
const autenticarBancoController = async (req, res) => {
    try {
        ...
        if (!userName || !password) {
            ...
            await grabarLog('BANCO GANADERO', userName || 'anonymous', "Pagos QR - Autenticación", mensaje, '', "/pago-qr-bg/prod/notification/login", process.env.PRD);
        ...
        if (!resultado.result) {
            const mensaje = `${resultado.message}` || 'Error Credenciales inválidas';
            await grabarLog('BANCO GANADERO', userName, "Pagos QR - Autenticación", mensaje, '', "/pago-qr-bg/prod/notification/login", process.env.PRD);
        ...
        await grabarLog('BANCO GANADERO', userName, "Pagos QR - Autenticación", 'Autenticación exitosa', '', "/pago-qr-bg/prod/notification/login", process.env.PRD);
        ...
        await grabarLog('BANCO GANADERO', req.body?.userName || 'anonymous', "Pagos QR - Autenticación", `Error: ${error.message}`, '', "/pago-qr-bg/prod/notification/login", process.env.PRD);
...
const registrarPagoController = async (req, res) => {
    try {
        ...
        if (!qrId || !transactionId || !payDate) {
            const mensaje = 'Error Parámetros de entrada insuficientes';
            await grabarLog(userCode, username, "Pagos QR - Registro Pago", mensaje, '', "/pago-qr-bg/prod/notification/payments", process.env.PRD);
            return res.status(400).json({
                result: 'COD001',
                message: mensaje
            });
        ...
        await grabarLog(userCode, username, "Pagos QR - Registro Pago", 'Pago registrado correctamente', '', "/pago-qr-bg/prod/notification/payments", process.env.PRD);
        return res.status(200).json({
            result: 'COD000',
        ...
        const userCode = req.user?.USERCODE || 'BANCO GANADERO';
        await grabarLog(userCode, username, "Pagos QR - Registro Pago", `Error: ${error.message}`, '', "/pago-qr-bg/prod/notification/payments", process.env.PRD);
        return res.status(500).json({
            result: 'COD003',
            message: mensaje
        });
...
const consultarEstadoOrdenController = async (req, res) => {
    try {
        ...
        if (!qrId) {
            const mensaje = 'Error Parámetros de entrada insuficientes';
            await grabarLog(userCode, username, "Pagos QR - Consulta Estado", mensaje, '', "/pago-qr-bg/prod/notification/estado-orden", process.env.PRD);
            return res.status(400).json({
                result: 'COD001',
                message: mensaje
            });
        ...
        await grabarLog(userCode, username, "Pagos QR - Consulta Estado", 'Consulta exitosa', '', "/pago-qr-bg/prod/notification/estado-orden", process.env.PRD);
        ...
        const username = req.user?.USERNAME || 'system';
        const userCode = req.user?.USERCODE || 'BANCO GANADERO';
        await grabarLog(userCode, username, "Pagos QR - Consulta Estado", `Error: ${error.message}`, '', "/pago-qr-bg/prod/notification/estado-orden", process.env.PRD);
        return res.status(500).json({
            result: 'COD003',
            message: mensaje
        });
...
const listarNotificacionesPagoController = async (req, res) => {
    try {
        ...
        if (!resultado.result) {
            ...
            await grabarLog(userCode, username, "Pagos QR - Listar Notificaciones", mensaje, '', "/pago-qr-bg/prod/notification/notificaciones", process.env.PRD);
            return res.status(500).json({
                result: 'COD003',
                message: mensaje
            });
        ...
        await grabarLog(userCode, username, "Pagos QR - Listar Notificaciones", 'Consulta exitosa', '', "/pago-qr-bg/prod/notification/notificaciones", process.env.PRD);
        return res.status(200).json({
            result: 'COD000',
            message: 'Consulta exitosa',
        ...
        const username = req.user?.USERNAME || 'system';
        const userCode = req.user?.USERCODE || 'BANCO GANADERO';
        await grabarLog(userCode, username, "Pagos QR - Listar Notificaciones", `Error: ${error.message}`, '', "/pago-qr-bg/prod/notification/notificaciones", process.env.PRD);
        return res.status(500).json({
            result: 'COD003',
            message: mensaje
        });
