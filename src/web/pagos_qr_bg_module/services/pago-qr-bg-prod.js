...
const BANK_USERNAME = process.env.BANK_USERNAME_PRD || "BANCO GANADERO";
const BANK_PASSWORD = process.env.BANK_PASSWORD_PRD || "Jk8a@@zP9!eRf2QsL";
...
const autenticarBanco = async (userName, password) => {
    try {
        console.log(`[PAGO-QR] Autenticando banco: ${userName}, ${password}`, `${userName !== BANK_USERNAME || password !== BANK_PASSWORD}`);
        if (userName.trim() !== BANK_USERNAME.trim() || password.trim() !== BANK_PASSWORD.trim()) {
            return {
                result: false,
