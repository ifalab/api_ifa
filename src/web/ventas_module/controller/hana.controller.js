...
const reportePendienteBySucursalesResume = async (fechainicio, fechafin, cliente, articulo, sucCodes) => {
    try {
        if (!connection) {
            await connectHANA()
        }
        // const query = `call ${ process.env.PRD }.IFA_SP_PENDING_DELIVERIES_GROUPED_RESUME()`
        const query = `call ${process.env.PRD}.IFASP_SAL_CALCULATE_PENDING_DELIVERIES_BY_MONTH('${fechainicio}','${fechafin}',${cliente},${articulo}, ${sucCodes})`
