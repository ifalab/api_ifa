...
const apiFacturacionProsin = {
    async facturacionProsin(body, user) {
        ...
        try {
            ...
            endTime = Date.now();
            grabarLog(user.USERCODE, user.USERNAME, "Facturacion Facturacion tiempo de respuesta con exito", 
                'tiempo de espera: '+`[${new Date().toISOString()}] Respuesta recibida. Tiempo transcurrido: ${endTime - startTime} ms`, `[${new Date().toISOString()}] Respuesta recibida. Tiempo transcurrido: ${endTime - startTime} ms`, "facturacion/facturacion", process.env.PRD)
    ...
    async facturacionExportacionProsin(body, user) {
        const startTime = Date.now();
        let endTime = Date.now();
        try {

            const response = await httpClientProsin.post('/api/sfl/FacturaComercialExportacion', body);
            endTime = Date.now();
            grabarLog(user.USERCODE, user.USERNAME, "Facturacion Exportacion tiempo de respuesta con exito", 'tiempo de espera: '+`[${new Date().toISOString()}] Respuesta recibida. Tiempo transcurrido: ${endTime - startTime} ms`, `[${new Date().toISOString()}] Respuesta recibida. Tiempo transcurrido: ${endTime - startTime} ms`, "facturacion/facturacion", process.env.PRD)
            // console.log({response})
            return {
                statusCode: response.status,
                data: response.data,
            ...
            console.log({ error })
            endTime = Date.now();
            grabarLog(user.USERCODE, user.USERNAME, "Facturacion Exportacion tiempo de respuesta con error", 'tiempo de espera: '+`[${new Date().toISOString()}] Respuesta recibida. Tiempo transcurrido: ${endTime - startTime} ms`, `[${new Date().toISOString()}] Respuesta recibida. Tiempo transcurrido: ${endTime - startTime} ms`, "facturacion/facturacion", process.env.PRD)
            if (error.response) {
                // Error específico de la API
                throw {
                    statusCode: error.response.status,
            ...
            throw {
                statusCode: 500,
                message: error.message + '. _currentUrl: https://lab2.laboratoriosifa.com:96/api/sfl/FacturaComercialExportacion' || 'Error desconocido en la solicitud facturacionProsin POST',
            };
        }
    },
};
